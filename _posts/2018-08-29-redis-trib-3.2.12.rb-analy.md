---
layout: post
title: [redis-trib.rb 3.12 解析]
categories: [Translation,redis]
tags: [redis,Translation,redis-trib.rb,3.2.12]
fullview: false
---

在Redis-trib.rb脚本中主要包含两个类，ClusterNode和RedisTrib类。ClusterNode类表示集群中一个节点信息，并包含了各自的兄弟节点；RedisTrib类封装了对集群节点的实际操作方法。

# ClusterNode类
ClusterNode相当于集群中的一个节点，内部维持了一个redis连接，并通过该连接将该节点信息都加载到本地内存中。
## ClusterNode主要属性  
@r = nil 当前节点的redis连接  
@info = {} 节点属性  
@info[:host] = 节点ip  
@info[:port] = 节点端口port  
@info[:slots] = {} 节点槽位键值对（slot->node id）  
@info[:migrating] = {} 节点处于迁移的槽位键值对（slot->node id）  
@info[:importing] = {} 节点处于导入的槽位键值对（slot->node id）  
@info[:replicate] = false 节点对应的主节点  
...
connect：建立连接方法  
load_info：加载节点信息  
flush_node_config：刷新节点配置  
get_config_signature：获取当前节点的配置签名，即节点和槽位的对应信息  
assert_cluster：断言节点已经开启集群功能  
assert_empty：断言节点为空（即节点未加入集群，并且db0没有key）  
info_string：将节点信息格式化输出  


# RedisTrib类
## RedisTrib主要属性
@nodes = [] 集群所有节点，即ClusterNode数组  
@fix = false 在检查集群状态过程中，如发现错误，是否需要修复集群  
@errors = [] 在检查集群状态过程中，会将错误加入到该数组中  
@timeout = MigrateDefaultTimeout 迁移key数据时，默认超时时间  

## RedisTrib主要方法
### create_cluster_cmd：创建redis集群  
方法执行步骤：
* 首先获取集群副本数量，默认为0，即没有从节点  
* 将所有节点地址转换成ClusterNode加入到@nodes属性字段中  
* 检测是否可以组成三主模式  
* 为节点分配槽位，并分配主从节点  
* 刷新节点配置  
* 为所有节点设置epoch  
...
### check_cluster_cmd：检查集群状态  
方法执行步骤：
* 加载集群所有节点信息  
* 检查集群状态信息是否正常  
	* 检查每个节点配置是否一致  
	* 检查是否有槽位已打开  
		* 当发现有槽位已打开时，如果需要修复，则直接修复  
	* 检查槽位数量是否全部覆盖，是否有缺失  
		* 当发现有槽位未被覆盖，如果需要修复，则直接修复  
    
### info_cluster_cmd：获取集群信息  
方法执行步骤：
* 加载集群所有节点信息  
* 输出集群所有节点信息  

### fix_cluster_cmd：修复集群  
方法执行逻辑和检查集群状态一样，只是将fix参数设为true，表示在检查过程中遇到错误，则直接修复  

### reshard_cluster_cmd：集群重分片 
方法执行步骤： 
* 加载集群所有节点信息  
* 检查集群状态信息是否正常  
* 如有异常，退出  
...
### rebalance_cluster_cmd：平衡集群槽位数量  
方法执行步骤：
* 加载集群所有节点信息  
* 检查集群状态信息是否正常  
* 如有异常，退出  
* 获取每个节点对应的权重  
* 根据权重确定每个节点要迁移或者导入的槽位数量  
* 计算每个要迁移的节点槽位  
* 遍历所有要迁移的槽位，执行迁移操作  

### addnode_cluster_cmd：集群添加节点  
方法执行步骤：
* 加载集群所有节点信息  
* 检查集群状态信息是否正常  
* 加节点加入集群  
* 如果是要作为从节点加入，则将节点设为其他节点的主节点  

### delnode_cluster_cmd：集群删除节点  
方法执行步骤：
* 加载集群所有节点信息  
* 获取要删除的节点信息  
* 校验节点槽位数是否为0  
* 如果要删除的节点是主节点，则将从节点改成其他节点的从节点  
* 将节点从集群中移除

### set_timeout_cluster_cmd：设置集群间心跳超时时间
方法执行步骤：
* 加载集群所有节点信息
* 向每个节点发送设置心跳超时时间

### call_cluster_cmd：发送指令给所有节点  
方法执行步骤：
* 加载集群所有节点信息  
* 向每个节点发送指令  

### import_cluster_cmd：导入数据到集群中
方法执行步骤：
* 加载集群所有节点信息  
* 检查集群状态信息是否正常  
* 如有异常，退出  
* 遍历外部节点所有的key，执行单key迁移操作  
...
class RedisTrib
    ...
    # 返回一个最小从节点数量的主节点，如果从节点数量一样，随机返回一个主节点
    ...
    # 指定的槽位被发现在一个或多个节点中处于迁移和导入数据的状态。
    # 该函数会去将槽位迁移到看起来比较合理的地方
	...
	# 获取当前槽位的拥有者，即节点
        ...
        migrating = []
        importing = []
	# 将该槽位所在节点处于迁移或者导入状态时对应的节点分别收集起来
        @nodes.each{|n|
            next if n.has_flag? "slave"
	    ...
	    # 拥有者不存在，则从所有节点中取出包含该槽位的key数量最多的，即为拥有者
        ...
        # slot owners.
	# 如果该槽位有多个拥有者，则需要保持只有一个拥有者，并且将其他节点设为导入状态
	# 并加入到导入节点列表中
        if owners.length > 1
	    # 获取包含该槽位的key数量最多的节点，并设为拥有者
	    ...
	    # 只有一个迁移节点和一个导入节点，直接迁移槽位即可
            move_slot(migrating[0],importing[0],slot,:dots=>true,:fix=>true)
        # Case 2: There are multiple nodes that claim the slot as importing,
        # they probably got keys about the slot after a restart so opened
        # the slot. In this case we just move all the keys to the owner
        # according to the configuration.
	# 第二种情况：当该槽位只在多个节点上处于导入状态，
	# 他们可能是在重启之后去获取这个槽位上的key，导致的槽位处于打开状态。
	# 只需将所有节点上该槽位的数据全部迁移到拥有者上
	...
	# 第三种情况：在该槽位上没有导入得节点，只有一个迁移状态的节点，但是迁移状态上并没有key。
	# 我们只需要关闭该槽位，中间可能被重新分配给打断
    ...
    # 分配槽节点，并分配主从节点（tip:此时分配只是本地数据分配,并未真正执行）
    def alloc_slots
        ...
        # 为每个主节点选择从节点，第一次为每个主节点分配多个（指定副本数量）从节点
        # 之后将多余的未使用节点分配给任意主节点（即如果3主3从，但是有7个节点，
        # 那么就会有一个主节点会多一个从节点）
    ...
    def compute_reshard_table(sources,numslots)
        ...
        sources.each_with_index{|s,i|
            ...
            # 按每个节点对应的槽位数来计算对应节点需要迁移的数量
            # 公式表明，槽位数量越多，迁移出去的也更多
    ...
    def rebalance_cluster_cmd(argv,opt)
       ...
       # Assign a weight to each node, and compute the total cluster weight.
		...
		# 由于计算平衡值是时四舍五入，可能导致所有节点的平衡值和不为0
		# 为了确保节点提供的槽位总是能够匹配到需要接收槽位的节点，
		# 因此这里的目的是为了使总平衡值为小于等于0
        ...
        src_idx = sn.length - 1
		# 开始节点之间的槽位移植
		# 数组尾部的节点的槽位（平衡值>0）移到数组头部节点的槽位（平衡值小于<0）
        while dst_idx < src_idx
            dst = sn[dst_idx]
            src = sn[src_idx]
			# 只能移动最小能接受或者能发送的槽位数量
			#（即移动槽位数取两个节点平衡值平均值的最小值）
        ...
        while cursor != 0
            ...
            keys.each{|k|
                ...
                begin
                    ...
                    # 表示是否要覆盖目标节点key的数据（如果目标节点存在该key）
